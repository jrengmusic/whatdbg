...
### Agents Participated
- **COUNSELOR** — problem framing (three distinct bugs surfaced during ARCHITECT's live DAW-plugin reproduction that smoke did not catch); role discipline reset after a /hey reminder (was about to prescribe `ps` verification); option filtering against BLESSED (evaluated throttle/no-op/architectural fix for the BP-retry O(N²) slowness — architectural fix scored highest on L/SSOT/Stateless/Encapsulation/Deterministic axes); decision gates for each fix lane; ODE-style ephemeral `[diag]` instrumentation rounds across shutdown, attach, process events, stdio drain
- **Pathfinder** — BP-machinery surface map (enumerated every `Session::*` BP method, every `session.*` call site in `BreakpointManager*`, every `state.hasNewModuleLoaded` / `state.lastLoadedImageName` set/read, existing `handleTargetEvent` event-bit coverage, liblldb `BreakpointCreateByLocation` / `SBBreakpoint` event API availability in vendored headers at `Resources/macos/liblldb/include/lldb/API/{SBTarget.h,SBBreakpoint.h}`, and codelldb's `breakpoints.rs:114` + `:511-527` + `:771-793` pattern as the reference)
- **Engineer** — all adapter edits (delegated by COUNSELOR with exact diffs): five instrumentation rounds, POSIX `kill` swap, `wasStopped` gate, Phase-1 `addBreakpointByLocation` rollout, Phase-2 `handleBreakpointEvent` + `processDeferredEvents` wiring; five smoke re-runs to confirm no regressions
- **Librarian** (from Sprint 20 context, not re-invoked this sprint) — `lldb-dap` `DAP::ConfigureIO` / `EventHelper::SendStdOutStdErr` pattern + upstream lldb-dap RequestHandler flag set (referenced, not re-queried)

### Files Modified (9 total)

- `Source/debug/Session_mac.cpp` — (1) `EndMode::terminate` switch branch (lines ~344-357) replaced `SBProcess::Kill()` with POSIX `::kill (pid, SIGKILL)` sourced from `State::getContext()->targetProcessId`; rationale: lldb's Kill blocks indefinitely on running attached macOS GUI targets (confirmed via `whatdbg.log` ending mid-`Kill` call, `Log.h:38` flushes per-write so the missing next-line is the call not returning); (2) added `<signal.h>` and `<cerrno>` system includes (lines ~6-8); (3) `Session::addBreakpointByLocation` implementation (lines ~428-464) — direct `target.BreakpointCreateByLocation (filePath, line)`, extracts `bp.GetID()` + first-location's line entry for `outResolvedLine`, returns `ok()` even when 0 locations (liblldb auto-resolves on module load); (4) TU-local `handleBreakpointEvent` (lines ~180+) — handles `eBreakpointEventTypeLocationsResolved` / `eBreakpointEventTypeLocationsAdded` from `SBBreakpoint::GetBreakpointEventTypeFromEvent`, sets State deferred flags; (5) `pollEvents` dispatch gains `else if (lldb::SBBreakpoint::EventIsBreakpointEvent (event))` branch; (6) ephemeral `[diag]` logging in `handleProcessEvent`, `Session::attach`, `Session::shutdown`, and `drainProcessStdio` for DAW-plugin runtime investigation
- `Source/debug/Session.h` — `Session::addBreakpointByLocation (filePath, line, outEngineId, outResolvedLine) noexcept` declaration inserted after `addBreakpoint` (line ~175) with doxygen block describing cross-platform semantics (Windows wraps offset chain, macOS delegates to liblldb)
- `Source/debug/Session.cpp` — Windows `Session::addBreakpointByLocation` implementation (lines ~465-492) wraps existing `getOffsetByLine` + `addBreakpoint (offset)` chain; returns `fail` when `getOffsetByLine` returns `notFound` (module-not-loaded) so BreakpointManager falls through to its pending-list retry path unchanged
- `Source/debug/BreakpointManager.h` — `onBreakpointLocationsResolved (engineId, resolvedLine) -> int` declared in public BP-event handler region with doxygen describing async resolution path
- `Source/debug/BreakpointManager.cpp` — `onBreakpointLocationsResolved` implementation: `engineToDap.count(engineId) > 0` → fetch dapId, `breakpoints.count(dapId) > 0` → update `BreakpointInfo::isVerified=true` + `line=resolvedLine`, return dapId (0 on miss)
- `Source/debug/BreakpointManagerHandlers.cpp` — `handleSetBreakpoints` new-BP block (lines ~136-199) replaced: calls `session.addBreakpointByLocation (resolvePath, line, &engineId, &resolvedLine)`; splits outcomes into `isImmediatelyResolved` (resolvedLine != 0 → verified), `isDeferredPending` (Mac lldb-tracked, engineId recorded but `pending` list NOT populated), and Windows-fail fallback (adds to `pending` → existing `onModuleLoad` retry path)
- `Source/Whatdbg.cpp` — (1) `processDeferredEvents` module-load block (lines ~215-244): captures `wasStopped = (state.executionState == stopped)` before BP resolution, moves `session.resume()` + `state.executionState = running` into an `if (wasStopped) { … }` guard; rationale: Windows `LoadModule` callback returns `DEBUG_STATUS_BREAK` so state was stopped; macOS `eBroadcastBitModulesLoaded` fires on running target and must not spuriously Continue; (2) new `if (state.hasBreakpointLocationsResolved)` deferred block — calls `breakpointManager.onBreakpointLocationsResolved (engineId, resolvedLine)` and emits DAP `breakpoint` event `reason="changed"`, `verified=true`, `line=resolvedLine`; (3) ephemeral `[diag]` logs in `run()` exit + output-emit paths
- `Source/WhatdbgHandlers.cpp` — ephemeral `[diag]` logging in `handleAttach` + `handleDisconnect` for DAW investigation (lines ~84, 91, 115)
- `Source/debug/State.h` — three new fields `bool hasBreakpointLocationsResolved { false }`, `std::uint32_t resolvedBreakpointEngineId { 0 }`, `std::uint32_t resolvedBreakpointLine { 0 }` placed adjacent to `hasNewModuleLoaded` with doxygen explaining set-site, consumer, and single-slot-is-acceptable rationale

### Alignment Check
- [x] **BLESSED principles followed** — the architectural refactor improves BLESSED vs pre-sprint baseline, per the Option 1 analysis:
  - **L (Lean):** removes the adapter-side O(N² × compile-units) retry loop on macOS (`onModuleLoad` iterates empty `pending` list on Mac since we no longer populate it); `addBreakpointByLocation` replaces the `getOffsetByLine → tryResolve → addBreakpoint(offset)` two-step with one call
  - **S (SSOT):** BP-resolution truth lives in liblldb on Mac (where it belongs) instead of being mirrored by our `pending` array + retry-on-every-module-event loop — no shadow state to drift
  - **S (Stateless):** pending-BP retry machinery was an adapter-side workaround for dbgeng's offset-only API; delegating to liblldb's internal BP manager restores "object manages its own state"
  - **E (Encapsulation):** `Session::addBreakpointByLocation (filePath, line, …)` is a tell-don't-ask interface — BreakpointManager no longer needs to know that Windows walks compile units internally or that macOS hands off to lldb; one public method, two private platform implementations
  - **D (Deterministic):** liblldb auto-resolves through a single deterministic path per module-load; previous behaviour depended on event ordering × retry-loop timing × `lineSearchWindow` walk — latent non-determinism
  - **B (Bound):** `SBBreakpoint` is a value-type handle with lldb-internal refcount; no raw owning pointer introduced
- [x] **NAMES.md adhered** — Rule −1 honoured. New identifiers introduced this sprint, each semantically grounded in existing vocabulary: `addBreakpointByLocation` (direct analog to existing `addBreakpoint (offset, …)`), `onBreakpointLocationsResolved` (matches `onBreakpointHit` / `onModuleLoad` event-handler pattern), `handleBreakpointEvent` (matches existing `handleTargetEvent` / `handleProcessEvent` TU-local dispatcher pattern), `hasBreakpointLocationsResolved` / `resolvedBreakpointEngineId` / `resolvedBreakpointLine` (match `hasNewModuleLoaded` / `lastLoadedImageName` + `hasBreakpointHit` / `breakpointEngineId` State field patterns). Locals `isImmediatelyResolved`, `isDeferredPending`, `wasStopped` — semantic, self-contained in one function each
- [x] **MANIFESTO.md applied** — JUCE-first (no custom UTF-8 sanitation, no manual pthread signal wrappers — we use POSIX `kill` and `signal.h` directly because that IS the native layer for Mac process termination); YAGNI (Phase-1/Phase-2 split — Phase 2 added only after Phase 1 proved the perf fix; queue-vs-single-slot State design deliberately chose single slot matching existing `hasBreakpointHit` precedent rather than introducing a new collection type); codelldb/lldb-dap patterns copied verbatim instead of re-invented (pathfinder cited `codelldb/src/codelldb/src/debug_session/breakpoints.rs:114` for the `set_source_breakpoints` → `BreakpointCreateByLocation` idiom, `:771-793` for the event-handler pattern)
- [x] **JRENG-CODING-STANDARD.md** — brace init throughout new code; `not`/`and`/`or` alternative tokens (zero `!`/`&&`/`||`); nested positive checks; `noexcept` on all new session + handler methods; `const` before type; `static` file-local handlers (no anonymous namespaces); `*`/`&` stick to type; `nullptr` comparisons explicit; the `SBProcess::Kill` → POSIX `kill` swap treats the POSIX surface as its own idiom (signal.h is its own pattern, not shoehorned into a JUCE wrapper)

### Problems Solved

**Problem 1 — `terminate` from nvim leaves the DAW alive.** ARCHITECT reported live from DAW-plugin debugging; whatdbg log truncated at `[diag] Session::shutdown calling process.Kill()` with no subsequent `Kill result` line. `Log.h:38` confirms `fflush` fires after every `logWrite`, so the missing line proves `SBProcess::Kill()` is stuck mid-call rather than returning-with-null-target. codelldb calls bare `process.kill()` identically (`launch.rs:405`), but in an async-pump architecture the kernel signal eventually lands; whatdbg's synchronous shutdown path completes `SBDebugger::Destroy` before the async Kill request is delivered, at which point the adapter exits and the kernel auto-detaches ptrace — DAW continues unmolested. Fix: POSIX `::kill (pid, SIGKILL)` at `Session_mac.cpp:~344` — unblockable, synchronous delivery, kills the target before adapter tear-down. Verified in smoke 02_attach + 04_pause: `[diag] … kill rc=0 errno=0`, fixtures die cleanly.

**Problem 2 — DAW takes ~1 minute to load with whatdbg attached.** After Problem 1 was resolved, second symptom surfaced. Root cause: `BreakpointManager::onModuleLoad` (called by `Whatdbg.cpp:223` on every `eBroadcastBitModulesLoaded` event) iterates pending BPs × `tryResolve` with `lineSearchWindow + 1` candidate lines × full-path + basename attempts × `Session::getOffsetByLine` which walks `target.GetNumModules() × GetNumCompileUnits()` per call. For a DAW with ~500 dylibs and 1 pending BP this is in the millions of DWARF line-entry lookups — O(N²) cumulative across all module-load events during dyld's initial load. codelldb avoids this entirely: `debug_session/breakpoints.rs:114` calls `SBTarget::BreakpointCreateByLocation` directly, liblldb's internal BP-location manager handles module-load auto-resolution incrementally. whatdbg inherited the offset-based pattern from Windows dbgeng (which has no file-based BP API). Fix: Phase 1 — `Session::addBreakpointByLocation` with macOS implementation calling `BreakpointCreateByLocation` directly; `handleSetBreakpoints` routes Mac BPs through the new method without populating the `pending` list, so `onModuleLoad` iterates empty on Mac. Windows path unchanged.

**Problem 3 — Spurious `process.Continue()` on every module-load event.** `Whatdbg.cpp:230` unconditionally re-resumed after processing `hasNewModuleLoaded`. Comment justified "LoadModule returned DEBUG_STATUS_BREAK" — Windows semantics: dbgeng's callback pauses the target, adapter must re-resume. On Mac, `eBroadcastBitModulesLoaded` fires from an already-running target — no pause occurred, so `Continue()` on an already-running process is wasteful and/or harmful. Pathfinder confirmed codelldb emits a DAP `module` event on module-load and nothing else (`debug_session.rs:1588`) — no resume, no state change. Fix: capture `const bool wasStopped { state.executionState == stopped }` before the BP-resolve block and gate the `session.resume()` call on it. Windows path preserved; Mac becomes no-op on module load.

**Problem 4 — async BP resolution on Mac loses the DAP `breakpoint changed` notification.** Phase 1 eliminated the retry loop but lldb's async resolution was silently dropped — `pollEvents` dispatch at `Session_mac.cpp:~325` routed only `SBProcess::EventIsProcessEvent` and `SBTarget::EventIsTargetEvent`; `SBBreakpoint`-broadcaster events fell off the end. Added `handleBreakpointEvent` TU-local handler + third dispatch branch, three State deferred fields, `processDeferredEvents` consumer block, and `BreakpointManager::onBreakpointLocationsResolved` updater. Matches codelldb's `breakpoints.rs:771-793` handler pattern: on `LocationsResolved` / `LocationsAdded`, extract BP ID + first location's line, emit DAP `breakpoint` event `reason=changed` `verified=true`. Gutter-update UX restored for the deferred-resolution path.

### Debts Paid
- None (DEBT.md does not exist at project root)

### Debts Deferred
- None — every issue surfaced during the sprint was resolved per ARCHITECT's "fix. no defer" directive from Sprint 20, carried forward. The ephemeral `[diag]` instrumentation added across `Session_mac.cpp`, `WhatdbgHandlers.cpp`, and `Whatdbg.cpp` is not a deferred debt — it is ongoing diagnostic support for the DAW-plugin investigation; ARCHITECT directs removal after the retest confirms the perf fix holds. Phase 2 of the BP refactor (async-resolution event handling) was originally presented as an optional split from Phase 1 but ARCHITECT said "go" → landed in the same sprint.
